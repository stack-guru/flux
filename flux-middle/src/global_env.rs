...
impl<'genv, 'tcx> GlobalEnv<'genv, 'tcx> {
    ...
    pub fn register_adt_def(&mut self, def_id: DefId, sorts: &[core::Sort], is_opaque: bool) {
        let sorts = sorts
            .iter()
        ...
        self.adt_defs
            .get_mut()
            .insert(def_id, ty::AdtDef::new(self.tcx.adt_def(def_id), sorts, is_opaque));
    }
    ...
    pub fn adt_def(&self, def_id: DefId) -> ty::AdtDef {
        ...
        self.adt_defs
            .borrow_mut()
            .entry(def_id)
            .or_insert_with(|| ty::AdtDef::new(self.tcx.adt_def(def_id), vec![], false))
            .clone()
    }
